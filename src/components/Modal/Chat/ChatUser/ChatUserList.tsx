...
const ChatUserList = (props: ChatUsersInfoTypes): JSX.Element => {
  ...
  const userProfileApiHandler = async () => {
    try {
      const response = await instance(`/users/profile/${props.nickname}`);
      if (response.status === 200) {
        setModalProps({ nickname: response.data.nickname });
  ...
  return (
    ...
    >
      <section className="flex items-center" onClick={userProfileApiHandler}>
        <img
